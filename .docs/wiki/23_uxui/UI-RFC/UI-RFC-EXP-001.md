...
### 1. Solicitud de exportación CSV

- request:
  - `POST /api/v1/mood-entries/export`
  - header: `Authorization: Bearer <access_token>`
  - body: `{ "period": string }`
- respuesta esperada (contrato futuro — no existe runtime):
  - `202 Accepted` + `downloadUrl` o redirect
  - archivo CSV generado asincrónicamente
- errores ожидаемые:
  - `NO_ENTRIES` — CSV con headers o mensaje claro
  - `GENERATION_IN_PROGRESS` — archivo en preparación
  - `SERVICE_UNAVAILABLE`
...
- respuesta esperada:
  ...
  - `Content-Disposition: attachment; filename="bitacora_<period>.csv"`

**Nota:** Endpoint de exportación es futuro. Contrato marcado como especulativo.
...
| click `Descargar CSV` | `POST /export` | `202` | `S02-GENERATING` luego `S02-SUCCESS` |
| sin registros | `POST /export` | `NO_ENTRIES` | genera CSV con headers |
| error de generación | `POST /export` | `ERROR` | `S03-ERROR` |
...
- Endpoint de exportación CSV es futuro — contrato especulativo;
