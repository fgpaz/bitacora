...
### 1. Lista de pacientes visibles para el profesional

- request:
  - `GET /api/v1/professional/patients`
  - header: `Authorization: Bearer <access_token>`
- respuesta esperada:
  - `patients[]` con `patientId`, `displayName`, `careLinkId`, `lastEntryAt`, `hasRecentAlert`, `canViewData`
  - `total` (int)
  - `hasMore` (bool)
- errores esperados:
  - `SERVICE_UNAVAILABLE` — error recuperable

### 2. Resumen de paciente específico

- request:
  - `GET /api/v1/professional/patients/{patientId}/summary?from=<DateOnly>&to=<DateOnly>`
  - header: `Authorization: Bearer <access_token>`
- respuesta esperada:
  - `averageMood`, `entryCount`, `dominantFactors[]`, `streakDays`
- errores esperados:
  - `403 Forbidden` — profesional no tiene acceso a ese paciente

### 3. Timeline de paciente específico

- request:
  - `GET /api/v1/professional/patients/{patientId}/timeline?from=<DateOnly>&to=<DateOnly>`
  - header: `Authorization: Bearer <access_token>`
- respuesta esperada:
  - `entries[]` con `entryId`, `recordDate`, `moodLevel`, `factorFlags`
  - `hasMore` (bool)

### 4. Alertas de paciente específico

- request:
  - `GET /api/v1/professional/patients/{patientId}/alerts?from=<DateOnly>&to=<DateOnly>`
  - header: `Authorization: Bearer <access_token>`
- respuesta esperada:
  - `alerts[]` con `alertType`, `triggeredAt`, `summary`

**Nota:** Endpoints verificados via mi-lsp (2026-04-12). Todos requieren `ProfessionalDataAccessAuthorizer` para verificar que el profesional tiene un CareLink activo con `canViewData=true` para ese paciente.

## Mapeo UX -> contrato

| Momento UI | Backend | Condición | Resultado visible |
| --- | --- | --- | --- |
| abre dashboard | `GET /professional/patients` | con datos | `S01-READY` |
| abre dashboard | `GET /professional/patients` | sin pacientes | `S02-EMPTY` |
| cambio de página | `GET /professional/patients` | con datos | `S01-PAGINATION` |
| fallo de carga | `GET /professional/patients` | error | `S03-ERROR` |
...
- la paginación es contenida y no rompe la jerarquía.
 - Endpoints de `professional/patients` existen en runtime — verificados via mi-lsp (2026-04-12);
