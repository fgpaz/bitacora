...
- respuesta esperada (contrato futuro — no existe runtime):
  ...
  - `status` (`revoked`)
  - `revokedAtUtc`
  - `cascades[]` — vínculos afectados
- errores ожидаемые:
  - `ALREADY_REVOKED`
  - `CASCADE_ERROR` — error en la transacción en cascada
  - `ERROR`
...
- respuesta esperada (contrato futuro):
  - `version`, `status`, `grantedAtUtc`, `patientStatus`

**Nota:** Estos contratos son especulativos hasta que existan en runtime.
...
| click `Revocar consentimiento` | `DELETE /consent` | `200` | `S03-SUCCESS` |
| consentimiento ya revocado | `DELETE /consent` | `ALREADY_REVOKED` | `S03-ALREADY` |
| error en cascada | `DELETE /consent` | `CASCADE_ERROR` | `S03-ERROR` |
| error recuperable | `DELETE /consent` | `ERROR` | `S03-ERROR` |
...
4. mobile y desktop conservan la misma lógica de pausa seria y serena.

## Dependencias abiertas

- Endpoint de revocación de consentimiento es futuro — contrato especulativo;
