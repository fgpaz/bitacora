# UX-VALIDATION-VIN-001 — Validación UX del slice VIN-001
...
- **Impacto:** las rutas profesionales (`/profesional/invitaciones`, `/profesional/pacientes`) no pueden validarse contra el backend real desde la sesión actual.
...
La validación UX del slice `VIN-001` queda en estado **`blocked`** en esta sesión.

- El build del frontend y del backend pasó (`next build` + `dotnet build`).
- Las rutas profesionales (`/profesional/invitaciones`, `/profesional/pacientes`, `/profesional/pacientes/[patientId]`) existen y compilan.
- No fue posible ejecutar validación interactiva real por los bloqueos documentados en `WEB-VAL-001` y `WEB-VAL-002`.
- La superficie backend de Vinculos está materializada (`GET /vinculos`, `GET /vinculos/active`, `POST /vinculos/accept`), lo cual se valida en smoke gate ( RF-VIN-001..004).
...
| --- | --- | --- | --- |
| WEB-VAL-001 | Critical | VIN-001 | Open |
| WEB-VAL-002 | Critical | VIN-001 | Open |
| WEB-VAL-003 | High | todas | Open |

## Retorno a la cadena canónica

El slice `VIN-001` tiene `UI-RFC + HANDOFF-*` completos bajo waiver de entrada a UI. Esa apertura no modifica el estado de validación: sigue `blocked` hasta que exista evidencia real de cohorte.

## Siguiente paso

Ejecutar cohorte híbrida `F` con sesión de navegador autenticada y `NEXT_PUBLIC_API_BASE_URL` configurada. Actualizar este documento con evidencia real.
