...
> Status: Wave A gap-closure completada GREEN el 2026-04-19; Wave B cutover Bitacora deployado en produccion el 2026-04-19 (`b0d876c`). Supabase/GoTrue fue retirado del runtime activo en la mini-wave de aceptacion.
...
Bitacora usa Zitadel como unico runtime de autenticacion. El frontend inicia sesion con OIDC Authorization Code + PKCE contra Zitadel; el backend valida Bearer JWT con metadata OIDC y JWKS RS256. La readiness productiva no depende de secretos Supabase/GoTrue.

La estrategia de migracion elegida fue link-on-first-login por `email_hash`. La aceptacion final renombra la columna fisica a `auth_subject` y elimina la columna de rollback.
...
- `07_baseline_tecnica.md` (auth runtime)
