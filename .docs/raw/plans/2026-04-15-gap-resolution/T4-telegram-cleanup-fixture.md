...
## Shared Context
**Goal:** Resolver GAP-01/02: el perfil QA `@tedi_responde` ya está vinculado al paciente `97a384ae` de la sesión 2026-04-14, impidiendo re-ejecutar TG-P01b (vinculación fresca) para el smoke user actual.
**Stack:** PostgreSQL (SSH `turismo` + docker exec), mi-telegram-cli
**Architecture:** La tabla `telegram_sessions` vincula `telegram_chat_id` con `patient_id`. Para desvincular, DELETE la fila de `@tedi_responde` (chat_id=8645234680) en producción DB.

## Locked Decisions
- El DELETE apunta al `patient_id = 97a384ae-...` (smoke user de 2026-04-14), NO al smoke user actual (1e9df465).
- El @tedi_responde se reasigna al smoke user actual (1e9df465) como perfil QA dedicado.
...
complexity: medium
done_when: "telegram_sessions no tiene fila con telegram_chat_id=8645234680; runbook creado en infra/runbooks/"
```
...
- @tedi_responde chat_id: `8645234680`
- Smoke user 2026-04-14 patient ID (a desvincular): `97a384ae` (buscar en DB para UUID completo)
...
```bash
ssh turismo "docker exec -i <container-name> psql -U bitacora -d bitacora_db -c \"DELETE FROM telegram_sessions WHERE telegram_chat_id = '8645234680';\""
```
...
```bash
ssh turismo "docker exec -i <container-name> psql -U bitacora -d bitacora_db -c \"SELECT * FROM telegram_sessions WHERE telegram_chat_id = '8645234680';\""
```
...
## Propósito
Desvincular el perfil QA @tedi_responde (chat_id=8645234680) antes de cada ciclo E2E que incluya TG-P01b.
...
```bash
ssh turismo "docker exec -i <CONTAINER> psql -U bitacora -d bitacora_db -c \"DELETE FROM telegram_sessions WHERE telegram_chat_id = '8645234680';\""
```
Resultado esperado: `DELETE 1`

### 3. Verificar
```bash
ssh turismo "docker exec -i <CONTAINER> psql -U bitacora -d bitacora_db -c \"SELECT count(*) FROM telegram_sessions WHERE telegram_chat_id = '8645234680';\""
```
...
## Verify
`ssh turismo "docker exec -i <container> psql -U bitacora -d bitacora_db -c \"SELECT count(*) FROM telegram_sessions WHERE telegram_chat_id = '8645234680';\""` → `count = 0`
