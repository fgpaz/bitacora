...
function PendingStatusCard({ invite, email }: { invite: InviteResponse; email: string }) {
  return (
    <div className={styles.card} role="status" aria-live="polite">
      <h3 className={styles.cardTitle}>Invitación enviada</h3>
      <p className={styles.cardText}>
        Se envió una invitación a <strong>{email}</strong>. Cuando se registre, aparecerá en tu
        lista de pacientes.
      </p>
      <dl className={styles.meta}>
        <div className={styles.metaRow}>
          <dt className={styles.metaKey}>ID de invitación</dt>
...
function ConflictResolutionCard({ email }: { email: string }) {
  return (
    <div className={`${styles.card} ${styles.cardConflict}`} role="alert" aria-live="assertive">
      <h3 className={styles.cardTitle}>Este paciente ya tiene una invitación o vínculo</h3>
      <p className={styles.cardText}>
        El correo <strong>{email}</strong> ya está vinculado a tu cuenta o tiene una invitación
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (code === 'ALREADY_EXISTS') {
        ...
        setStatus('error');
        setErrorMsg(err instanceof Error ? err.message : 'No se pudo enviar la invitación.');
      }
    }
  }
